...
Release binaries are published with a `SHA256SUMS` manifest. The installer verifies the downloaded binary against that manifest before installing it. Release assets and checksums are also covered by GitHub artifact attestations generated by the release workflow.
...
Reports the active binary, config and index paths, project directory, shell integration, and stale indexed project paths.
